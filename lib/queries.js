...
export const TRADES_RECENT = {
  sql: `
    SELECT
        ...
        edge_pct,
        clv_cents,
        -- Weather trades populate via the settlements table (latest_*
        -- columns from the MV LATERAL join).  Mention trades have their
        -- settlement applied directly to trades.* by
        -- core.mentions.settlement_audit (no settlements row).  Use
        -- COALESCE so the ledger renders both flows consistently.
        -- See incident 2026-05-26.
        COALESCE(latest_actual_value, actual_value) AS actual_value,
        COALESCE(latest_won, won)                   AS won,
        COALESCE(latest_pnl, pnl)                   AS pnl,
        crps_cal,
        crps_skill_score,
        in_predictive_50pi,
        in_predictive_90pi,
...
export const STRATEGY_COMPARE = {
  sql: `
    SELECT
        ...
        market_type,
        COUNT(*)                                                              AS n_settled,
        SUM(CASE WHEN COALESCE(latest_won, won) = TRUE  THEN 1 ELSE 0 END)    AS n_won,
        SUM(CASE WHEN COALESCE(latest_won, won) = FALSE THEN 1 ELSE 0 END)    AS n_lost,
        COALESCE(SUM(COALESCE(latest_pnl, pnl)), 0)::REAL                     AS total_pnl_net,
    ...
    FROM analytics.mv_trades_full
    WHERE COALESCE(latest_settled_at, settled_at) IS NOT NULL AND strategy IS NOT NULL
...
export const REALIZED_SUMMARY = {
  sql: `
    SELECT
        ...
        COALESCE(SUM(fees_paid), 0)::REAL                                     AS total_fees_paid
    FROM analytics.mv_trades_full
    WHERE COALESCE(latest_settled_at, settled_at) IS NOT NULL
...
export const BANKROLL_SUMMARY = {
  sql: `
    ...
    agg AS (
      -- COALESCE(latest_*, *) so weather orphans (settlement row exists,
      -- trades.* mirror update missed) and any other reconciler-lag
      -- conditions still aggregate correctly.  See 2026-05-28 audit.
      SELECT
        COUNT(*)                                                            AS n_trades,
        COUNT(*) FILTER (WHERE COALESCE(latest_settled_at, settled_at) IS NOT NULL) AS n_settled,
        COUNT(*) FILTER (WHERE COALESCE(latest_settled_at, settled_at) IS NULL
                          AND trade_status = 'open')                        AS n_open,
        COUNT(*) FILTER (WHERE COALESCE(latest_won, won) = TRUE)            AS n_won,
        COUNT(*) FILTER (WHERE COALESCE(latest_won, won) = FALSE)           AS n_lost,

        COUNT(*) FILTER (WHERE mode = 'paper')                              AS n_paper,
        COUNT(*) FILTER (WHERE mode = 'live')                               AS n_live,

        COALESCE(SUM(COALESCE(latest_pnl, pnl))
          FILTER (WHERE COALESCE(latest_settled_at, settled_at) IS NOT NULL), 0)::REAL AS total_pnl_net,
        COALESCE(SUM(pnl_gross)
          FILTER (WHERE COALESCE(latest_settled_at, settled_at) IS NOT NULL), 0)::REAL AS total_pnl_gross,
        COALESCE(SUM(fees_paid)
          FILTER (WHERE COALESCE(latest_settled_at, settled_at) IS NOT NULL), 0)::REAL AS total_fees,

        COALESCE(SUM(COALESCE(latest_pnl, pnl))
          FILTER (WHERE COALESCE(latest_settled_at, settled_at) IS NOT NULL
        ...
        COALESCE(SUM(intent_size_usd)
          FILTER (WHERE COALESCE(latest_settled_at, settled_at) IS NOT NULL), 0)::REAL AS total_size_settled,
        COALESCE(SUM(intent_size_usd)
          FILTER (WHERE COALESCE(latest_settled_at, settled_at) IS NULL
...
export const BANKROLL_CURVE = {
  sql: `
    SELECT
        ...
        trade_id,
        decided_at,
        COALESCE(latest_settled_at, settled_at)   AS settled_at,
        mode,
        market_type,
        city,
        bet_side,
    ...
    WHERE account_id = '${BANKROLL_ACCOUNT_ID}'
      AND mode IN ('paper', 'live')
      AND COALESCE(latest_settled_at, settled_at) IS NOT NULL
      AND decided_at >= '${TRACKING_EPOCH_START}'::TIMESTAMPTZ
    ORDER BY COALESCE(latest_settled_at, settled_at) ASC
    LIMIT 5000
  `,
  defaultLimit: null,
};
...
export const MENTIONS_TRADES_RECENT = {
  sql: `
    SELECT id, decided_at, settled_at, status, strategy,
           ...
           NOW() AS _mv_refreshed_at
      FROM trades
     WHERE strategy IN ('nqe_sweep_v1','sports_bandit_v1','sports_bandit_v2',
                        'trump_recurring_v1','hannity_cover_v1','brand_launch_v1',
                        'trump_poisson_v1','trump_wide_no_v1','trump_dip_yes_v1')
     ORDER BY decided_at DESC
     LIMIT 100
  `,
  defaultLimit: null,
